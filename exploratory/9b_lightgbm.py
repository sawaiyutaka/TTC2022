...
model = lgb.LGBMClassifier(objective='binary',
                           ...
                           learning_rate=0.01,
                           n_estimators=100000, )
feat_selector = BorutaPyForLGB(model, n_estimators='auto', perc=90, two_step=False, verbose=2, random_state=42)
...
plt.figure()
explainer = shap.TreeExplainer(model=model, data=X_train_selected)
shap_values = explainer.shap_values(X=X_train_selected)
shap.summary_plot(shap_values=shap_values,
                  features=X_train_selected,
                  feature_names=X_train_selected.columns,
                  max_display=len(X_selected.columns))
# shap.summary_plot(shap_values, X_selected, max_display=len(X_selected.columns))
